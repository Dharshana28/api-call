...
export default function App() {
  ...
  const getbyid = async (id) => {
    if (!window.confirm("Data get by id")) return;
    try {
      const display = await axios.get(`${BASE_URL}/books/${id}`);
      setForm({
        title: display.data.title,
        author: display.data.author,
        price: display.data.price,
        publicationYear: display.data.publicationYear,
      });
    } catch (e) {
  ...
  const deleteall = async () => {
    if (!window.confirm("Delete All the books?")) return;
    try {
            ...
            books.map((b) => (
              <tr key={b.id}>
                ...
                <td>
                  ...
                  <button onClick={() => getbyid(b.id)}>Get</button>
                </td>
              </tr>
            ))}
        </tbody>
